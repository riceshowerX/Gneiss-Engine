...
def main():
    ...
    try:
        ...
        print(f"  Size: {metadata['basic'].get('width')}x{metadata['basic'].get('height')}")
    ...
    try:
        ...
        if gps_coords:
            ...
            print(f"  Google Maps link: https://maps.google.com/?q={gps_coords['latitude']},{gps_coords['longitude']}")
        else:
    ...
    try:
        ...
        if success:
            ...
            if not stripped_exif:
                print("Verification successful: No EXIF data found in the stripped image.")
            else:
                print("Warning: Some EXIF data may still be present in the stripped image.")
        else:
    ...
    try:
        ...
        if success:
            print(f"Successfully copied metadata from {source_image_path.name} to {target_image_path.name}")
            ...
            if source_exif and target_exif:
                ...
                matches = sum(1 for tag in important_tags 
                             if tag in source_exif and tag in target_exif 
                             and source_exif[tag] == target_exif[tag])
                
                if matches > 0:
                    print(f"Verification successful: {matches} key EXIF fields match between source and target.")
                else:
                    print("Warning: No matching EXIF fields found between source and target.")
            else:
                print("Warning: Could not verify metadata copying.")
        else:
            print(f"Failed to copy metadata from {source_image_path.name} to {target_image_path.name}")
