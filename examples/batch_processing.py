...
def main():
    ...
    try:
        ...
        print(f"Successfully converted {success_count} out of {len(image_paths)} images to WEBP format.")
    ...
    try:
        ...
        success_count = sum(1 for result in results.values() if not isinstance(result, Exception))
        print(f"Successfully watermarked {success_count} out of {len(image_paths)} images.")
    ...
    try:
        ...
        success_count = sum(1 for result in results.values() if not isinstance(result, Exception))
        print(f"Successfully processed {success_count} out of {len(image_paths)} images with custom operation.")
