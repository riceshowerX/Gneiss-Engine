...
def get_gps_coordinates(image_path: Union[str, Path]) -> Optional[Dict[str, float]]:
    ...
    try:
        latitude = convert_to_decimal(gps_info['GPSLatitude'], gps_info.get('GPSLatitudeRef', 'N'))
        longitude = convert_to_decimal(gps_info['GPSLongitude'], gps_info.get('GPSLongitudeRef', 'E'))
...
def strip_all_metadata(image_path: Union[str, Path], output_path: Optional[Union[str, Path]] = None) -> bool:
    """
